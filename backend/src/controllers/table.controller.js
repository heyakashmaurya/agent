...
import { parseBoolean, parsePositiveInt, parseString, isObjectId, sendError, sendSuccess, logControllerError } from "./_controllerUtils.js";
...
const canonicalLocation = (value) => {
  const text = parseString(value, "Indoor");
  return LOCATION_ALIASES[text.toLowerCase()] || text;
};

export const createTable = async (req, res, next) => {
  try {
    const b = req.body || {};
    const tableNumber = parsePositiveInt(b.tableNumber ?? b.number, null, 9999);
    const capacity = parsePositiveInt(b.capacity, null, 50);
    const location = canonicalLocation(b.location ?? b.section);
    ...
    if (!LOCATIONS.includes(location)) return sendError(res, { status: 400, message: "Invalid table location. Use Indoor, Outdoor, Window or Private." });
...
export const getTables = async (req, res, next) => {
  try {
    ...
    if (req.query?.location) {
      const location = canonicalLocation(req.query.location);
    ...
    return sendSuccess(res, { data: tables || [], meta: { total: Array.isArray(tables) ? tables.length : 0 }, message: "Tables retrieved successfully." });
  } catch (error) {
...
export const getTableById = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, { data: await tableService.getTableById(id) });
  } catch (error) {
...
export const updateTable = async (req, res, next) => {
  try {
    ...
    if (b.tableNumber !== undefined) {
      const value = parsePositiveInt(b.tableNumber, null, 9999);
      if (!value) return sendError(res, { status: 400, message: "Invalid table number." });
      payload.tableNumber = value;
    }
    if (b.tableName !== undefined) payload.tableName = parseString(b.tableName);
    if (b.capacity !== undefined) {
      const value = parsePositiveInt(b.capacity, null, 50);
      if (!value) return sendError(res, { status: 400, message: "Capacity must be between 1 and 50." });
      payload.capacity = value;
    }
    if (b.location !== undefined || b.section !== undefined) {
      const location = canonicalLocation(b.location ?? b.section);
      if (!LOCATIONS.includes(location)) return sendError(res, { status: 400, message: "Invalid table location. Use Indoor, Outdoor, Window or Private." });
      payload.location = location;
    }
    if (b.floor !== undefined) {
      const value = parsePositiveInt(b.floor, null, 100);
      if (!value) return sendError(res, { status: 400, message: "Invalid floor." });
      payload.floor = value;
    }
    if (b.status !== undefined) {
      const value = parseString(b.status);
      if (!STATUSES.includes(value)) return sendError(res, { status: 400, message: "Invalid table status." });
...
export const updateTableStatus = async (req, res, next) => {
  try {
    const id = parseString(req.params?.id);
    if (!isObjectId(id)) return sendError(res, { status: 400, message: "Invalid table ID." });
    const raw = parseString(req.body?.status);
    const status = raw.toLowerCase() === "cleaning" ? "Maintenance" : raw ? raw.charAt(0).toUpperCase() + raw.slice(1).toLowerCase() : raw;
    if (!STATUSES.includes(status)) return sendError(res, { status: 400, message: "Invalid table status. Use Available, Reserved, Occupied or Maintenance." });
