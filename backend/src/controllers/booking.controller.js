...
export const createBookingController = async (req, res, next) => {
  try {
    ...
    const email = parseString(b.email).toLowerCase();
    const bookingDate = parseString(b.bookingDate || b.date);
    const startTime = parseString(b.startTime || b.time);
    const guestCount = parsePositiveInt(b.guestCount ?? b.guests ?? b.partySize, null, 100);
    ...
    if (!guestCount) return sendError(res, { status: 400, message: "Guest count must be a whole number between 1 and 100." });
    if (!BOOKING_SOURCES.includes(bookingSource)) return sendError(res, { status: 400, message: "Invalid booking source." });
    if (tableId && !isObjectId(tableId)) return sendError(res, { status: 400, message: "Invalid table ID." });

    const result = await createBooking({
      name, phone, email, bookingDate, startTime, guestCount,
    ...
    if (!result?.success) return sendError(res, { status: 409, message: result?.message || "Unable to create booking.", details: { booking: result?.booking || null } });
    return sendSuccess(res, { status: 201, message: result.message || "Booking created successfully.", data: { booking: result.booking } });
  } catch (error) {
...
export const listBookingsController = async (req, res, next) => {
  try {
    ...
    const result = await listBookings({ bookingDate: parseString(req.query?.bookingDate) || undefined, status, customer, bookingSource, paymentStatus, page, limit });
    return sendSuccess(res, { message: result?.message || "Bookings retrieved successfully.", data: result?.bookings || [], meta: { total: result?.total || 0, page: result?.page || page, limit: result?.limit || limit, totalPages: result?.totalPages || 0 } });
  } catch (error) {
...
export const checkBookingAvailabilityController = async (req, res, next) => {
  try {
    ...
    if (!bookingDate || !isValidDateString(bookingDate)) return sendError(res, { status: 400, message: "A valid booking date is required." });
    if (!isValidTime(startTime)) return sendError(res, { status: 400, message: "Booking time must use HH:mm format." });
    if (!guestCount) return sendError(res, { status: 400, message: "Guest count must be between 1 and 100." });
    ...
    return sendSuccess(res, { data: { available: Boolean(result?.available), table: result?.table || null, endTime: result?.endTime || null, reason: result?.reason || null }, message: result?.available ? "Table is available." : (result?.reason || "No table is available.") });
  } catch (error) {
...
export const updateBookingController = async (req, res, next) => {
  try {
    ...
    const payload = {
      ...
      phone: b.phone !== undefined ? parseString(b.phone) : (b.customerPhone !== undefined ? parseString(b.customerPhone) : undefined),
      ...
      guestCount: b.guestCount !== undefined ? parsePositiveInt(b.guestCount, null, 100) : undefined,
    ...
    if (payload.tableId !== undefined && payload.tableId && !isObjectId(payload.tableId)) return sendError(res, { status: 400, message: "Invalid table ID." });
    ...
    if (payload.startTime !== undefined && !isValidTime(payload.startTime)) return sendError(res, { status: 400, message: "Invalid booking time. Use HH:mm." });
    if (payload.guestCount === null) return sendError(res, { status: 400, message: "Guest count must be between 1 and 100." });
...
export const cancelBookingController = async (req, res, next) => {
  try {
    ...
    const actorByRole = { Owner: "admin", Manager: "restaurant", Staff: "restaurant" };
    const cancelledBy = actorByRole[req.user?.role];
    if (!cancelledBy) return sendError(res, { status: 403, message: "Your account role cannot cancel bookings." });
    const result = await cancelBooking({ bookingId, reason, cancelledBy, cancelledByUser: req.user?._id || req.user?.id || null });
    if (!result?.success) return sendError(res, { status: /not found/i.test(result?.message || "") ? 404 : 409, message: result?.message || "Unable to cancel booking.", details: { booking: result?.booking || null } });
    return sendSuccess(res, { data: { booking: result.booking }, message: result.message || "Booking cancelled successfully." });
  } catch (error) {
...
export default { createBookingController, listBookingsController, getBookingController, checkBookingAvailabilityController, updateBookingController, cancelBookingController };
